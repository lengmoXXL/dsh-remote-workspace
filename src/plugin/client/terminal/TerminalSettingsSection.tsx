...
const STEPS: readonly StepRow[] = [
  { key: 'fontSize', label: 'settings.fontSize', show: value => `${String(value)} px` },
  { key: 'lineHeight', label: 'settings.lineHeight', show: value => value.toFixed(1) },
  { key: 'scrollback', label: 'settings.scrollback', show: value => String(value) },
]
...
export function TerminalSettingsSection({ t }: PropsLocale<TerminalNamespace>): ReactNode {
  ...
  const font = TERMINAL_FONTS.find(entry => entry.stack === settings.fontFamily) ?? DEFAULT_TERMINAL_FONT
  ...
  return (
    <div className={css.section}>
      <div className={css.head}><h3 className={css.title}>{t('settings.label')}</h3></div>
