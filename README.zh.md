...
- PTC program host 是同一个 Release 里的第二个二进制——一个 V8 引擎，比 agent 大得多。机器一连上就在后台下载并上传，
  因此那台机器第一次运行程序时不必等它。
...
- agent 可以在**开着标签的**终端里干活：列出终端、读输出、输入文本和按键（包括 `ctrl+c`）、等待输出出现。它不会
  创建或关闭终端。
- 在工作区所属的那个世界里运行 PTC 程序（`run_code`）。指向被路由工作区的程序会在那台机器上运行，用的是本插件装
  在 agent 旁边、内嵌 V8 的 program host，所以那台机器不需要 Node。跨线的只有声明过的 bindings：在远端，程序只能
  通过它调用的工具去碰文件系统和 shell，而不是靠 Node 自带的 API。
