...
*/
const DOWNLOAD_STALL_TIMEOUT_MS = 3e4;
/**
...
async function fetchOverHttps(url) {
	...
	try {
		...
		if (reader === void 0) throw new Error("the download carried no body");
		const chunks = [];
		for (;;) {
			watch(DOWNLOAD_STALL_TIMEOUT_MS, `no data for ${String(DOWNLOAD_STALL_TIMEOUT_MS)}ms after ${String(chunks.length)} chunks`);
