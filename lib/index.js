...
function createNodeConnections(deps = {}) {
	...
	const openTransport = deps.openTransport ?? defaultOpenTransport({
		...
		agentVersion: deps.agentVersion ?? "0.0.5",
		cacheDir: deps.cacheDir,
...
*/
async function translateRemoteArgv(channel, argv) {
	...
	for (let index = 1; index < translated.length; index += 1) {
		...
		if (value === void 0 || !value.startsWith("/") || !value.includes(HOST_ASSET_SEGMENT)) continue;
...
*/
async function stageHostAsset(channel, hostPath) {
