...
*/
const PTC_HOST_MEMBER = "dsh-ptc-host";
...
* @throws when the machine reports a platform or architecture with no asset.
*/
function ptcHostAssetName(platform, arch) {
...
* @throws when the machine reports a platform or architecture with no asset.
*/
function binaryAssetName(binary, platform, arch) {
	const os = PLATFORMS[platform.trim().toLowerCase()];
	const cpu = ARCHITECTURES[arch.trim().toLowerCase()];
	if (os === void 0 || cpu === void 0) throw new Error(`the machine reports platform "${platform.trim()}" and architecture "${arch.trim()}", which has no ${binary} release; it ships for Linux and Darwin on x86_64`);
...
* link.
*/
const DOWNLOAD_ANSWER_TIMEOUT_MS = 2e4;
/**
...
function downloadAddresses(url) {
...
async function download(fetcher, url) {
	...
	throw new Error(`downloading ${url} failed at every address tried (${addresses.join(", ")}): ${last instanceof Error ? last.message : String(last)}`, { cause: last });
}
/**
* The real HTTPS GET, refusing any non-2xx answer and giving up on one that
* never comes.
*
* The timeout covers the answer only. A body that has started arriving is left
* to run, because aborting it would break a slow link that is working, and a
* body that stalls halfway is what the HTTP client's own inactivity timeout is
* for.
*/
async function fetchOverHttps(url) {
	const controller = new AbortController();
	const timer = setTimeout(() => {
		controller.abort(/* @__PURE__ */ new Error(`no answer within ${String(DOWNLOAD_ANSWER_TIMEOUT_MS)}ms`));
	}, DOWNLOAD_ANSWER_TIMEOUT_MS);
	try {
...
*/
function archiveMember(archive, sourceUrl, member) {
	let tar;
	try {
	...
	for (let offset = 0; offset + TAR_BLOCK <= tar.length;) {
		...
		if ((type === "0" || type === "\0") && name.split("/").pop() === member) return tar.subarray(start, start + size);
...
*/
const READ_PTC_HOST = "test -x \"$HOME/.dsh/remote-agent/dsh-ptc-host\" && cat \"$HOME/.dsh/remote-agent/ptc-host.json\" 2>/dev/null || true";
...
const UPLOAD_PTC_HOST = "cat > \"$HOME/.dsh/remote-agent/dsh-ptc-host.new\" && chmod 755 \"$HOME/.dsh/remote-agent/dsh-ptc-host.new\" && mv \"$HOME/.dsh/remote-agent/dsh-ptc-host.new\" \"$HOME/.dsh/remote-agent/dsh-ptc-host\"";
...
async function installedVersion(run, ssh) {
	...
	return result.code === 0 ? markerVersion(result.stdout) : void 0;
}
...
*   or the upload fails.
*/
async function ensurePtcHost(options) {
	const run = options.run ?? runSsh;
	const resolveBinary = options.resolveBinary ?? resolveAgentBinary;
	...
	const binary = await resolveBinary({
		version,
		assetName: ptcHostAssetName(platform?.trim() ?? "", arch?.trim() ?? ""),
		member: PTC_HOST_MEMBER,
		cacheDir
	});
	await runChecked(run, ssh, ENSURE_DIR, `could not create ~/.dsh/remote-agent on "${ssh.target}"`);
	await runChecked(run, ssh, UPLOAD_PTC_HOST, `could not install the PTC program host on "${ssh.target}"`, binary);
	await runChecked(run, ssh, WRITE_PTC_HOST, `could not record the installed PTC program host on "${ssh.target}"`, JSON.stringify({ version }));
}
//#endregion
...
function createNodeConnections(deps = {}) {
	...
	const agentVersion = deps.agentVersion ?? "0.0.6";
	...
	*/
	const ptcHostFor = (entry, record) => {
		if (entry.ptcHost !== void 0) return entry.ptcHost;
		const home = entry.info?.homedir;
		if (home === void 0) return Promise.reject(/* @__PURE__ */ new Error(`"${record.title}" has not reported its home directory`));
		const path = posix.join(home, ...PTC_HOST_SEGMENT);
		if (record.transport.kind !== "ssh") return Promise.resolve(path);
		if (cacheDir === void 0) return Promise.reject(/* @__PURE__ */ new Error("installing a PTC program host needs the plugin data directory"));
		const attempt = installPtcHost({
			ssh: record.transport,
		...
		attempt.catch(() => {
			if (entry.ptcHost === attempt) entry.ptcHost = void 0;
		});
		return attempt;
	};
	/**
	...
	*/
	...
	const connectRecord = async (record) => {
		...
		const attempt = (async () => {
			try {
				...
				entry.record = record;
				entry.state = "ready";
				if (record.transport.kind === "ssh") ptcHostFor(entry, record).catch(() => {});
				opened.exited?.then(() => {
	...
	return {
		...
		async ptcHost(nodeId) {
			const entry = entries.get(nodeId);
			if (entry === void 0 || entry.state !== "ready" || entry.record === void 0) throw new Error(`remote node "${nodeId}" is not connected`);
...
*/
function createRemoteHandle(deps, channel, nodeId, remoteCwd, spec) {
	...
	let procId;
	let offPipe;
	const control = spec.stdio.control === "pipe" ? new Duplex({
		read() {},
		write(chunk, _encoding, callback) {
			...
			channel.request("sp.writeControl", {
				procId: id,
				data
			}).then(() => {
				callback();
	...
	const deliverPipeFrame = (frame) => {
		...
		(frame.stream === "stdout" ? stdoutPipe : stderrPipe)?.write(bytes);
	};
	if (stdoutPipe !== void 0 || stderrPipe !== void 0 || control !== void 0) offPipe = channel.onPipeFrame((frame) => {
		if (procId === void 0) {
	...
	const run = async () => {
		...
		try {
			id = (await channel.request("sp.spawn", {
				...
				...spec.stdio.control === "pipe" ? { control: "pipe" } : {},
				graceMs: spec.graceMs,
				...spec.env === void 0 ? {} : { env: definedEnv(spec.env) }
			})).procId;
			procId = id;
			for (const frame of bufferedFrames.splice(0)) deliverPipeFrame(frame);
			for (const data of pendingControl.splice(0)) settled(channel.request("sp.writeControl", {
				procId: id,
				data
			}));
		} catch (error) {
...
const SYSTEM_EXECUTABLE_PREFIXES = [
	"/bin/",
	"/sbin/",
	"/usr/bin/",
	"/usr/sbin/",
...
*/
function stripHostSandbox(argv) {
	const head = argv[0];
	if (head === void 0 || !HOST_SANDBOX_RUNNERS.includes(basename(head))) return argv;
...
*/
function ptcBootstrapIndex(argv) {
	if (argv.length < 3) return void 0;
	const limit = argv[argv.length - 1];
	const bootstrap = argv[argv.length - 2];
	if (limit === void 0 || bootstrap === void 0) return void 0;
	if (!/^[1-9][0-9]*$/.test(limit)) return void 0;
	if (!bootstrap.includes(PTC_PACKAGE_SEGMENT)) return void 0;
	if (bootstrap.slice(bootstrap.lastIndexOf("/") + 1) !== PTC_BOOTSTRAP) return void 0;
...
*/
async function translateRemoteArgv(deps, nodeId, channel, argv) {
	const translated = [...stripHostSandbox(argv)];
	const bootstrap = ptcBootstrapIndex(translated);
	if (bootstrap !== void 0) return [
		await deps.ptcHost(nodeId),
		...translated.slice(1, bootstrap),
		translated[bootstrap + 1]
	];
	const head = translated[0];
	if (head !== void 0 && head.startsWith("/") && !SYSTEM_EXECUTABLE_PREFIXES.some((prefix) => head.startsWith(prefix)) && await missingOnNode(channel, head)) translated[0] = basename(head);
	return translated;
}
/** Whether the node has no readable entry at one absolute path. */
async function missingOnNode(channel, path) {
	try {
...
function createRoutingSubprocessRuntime(deps) {
	...
	return {
		...
		spawn(spec) {
			...
			return createRemoteHandle(deps, remote.channel, remote.nodeId, remote.remotePath, {
				...spec,
